...
public class MateriaDAO implements IMateriaDAO {
	...
	public List<Fecha_Cursada> getPresentismos(Usuario usr) {
		List<Fecha_Cursada> fecha_cursadas = new ArrayList<Fecha_Cursada>();
		Query query = getSessionFactory()
				.getCurrentSession()
				.createQuery(
						"from Fecha_Cursada fc Where fc.dia_Cursada.cursada.usuario = :usr");
		...
		List list = query.list();
		for (Object obj : list) {
			fecha_cursadas.add((Fecha_Cursada) obj);
		}

		return fecha_cursadas;
	}
	...
	public void setPresentismo(List<Fecha_Cursada> presentismosSeleccionados,
		...
		for (Fecha_Cursada fecha_Cursada : presentismosSeleccionados) {
			getSessionFactory().getCurrentSession().update(fecha_Cursada);
		}
	}
}
